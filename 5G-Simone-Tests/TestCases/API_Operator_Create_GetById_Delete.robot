...
*** Test Cases ***
Validate "Operator Create" "GetById" and "Delete"
    [Tags]      API         Get         Post        Operator
    API_Keywords.Tenant Admin Login
    API_Keywords.Create Base Session
    API_Keywords.Operator Post Request
    API_Keywords.Validate "Create" Post Request is Successful
    API_Keywords.Create Get Request Session For "Operator_GetById"
    API_Keywords.Validate Get Request is Successful
    DELETE On Session           ${API Session Alias}            ${Operator_Delete}${Operator_ID}          headers=${headers}
